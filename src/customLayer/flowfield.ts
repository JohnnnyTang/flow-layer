...
export class FlowFieldManager {
    ...
    async Prepare(gl: WebGL2RenderingContext, ) {
        ...
        for (let i = 0; i < MAX_STREAMLINE_NUM; i++) {
            particleCountdownArray[i] = Math.floor(rand(this.controller!.segmentNum, this.controller!.fullLife));
        }
    ...
    private TransMercator(bbox: [number, number, number, number]) {
        ...
        console.log(min.toLngLat(), max.toLngLat());
    ...
    tickRender(gl: WebGL2RenderingContext) {
        ...
        gl.drawArraysInstanced(gl.TRIANGLE_STRIP, 0, (this.segmentNum - 1) * 2, this.streamline * this.zoomRate);
        gl.disable(gl.BLEND);
